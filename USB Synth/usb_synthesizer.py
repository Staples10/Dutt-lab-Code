...
class USB_RFGenerator(Device):
    _DEFAULT_SETTINGS = Parameter([
        ...
        Parameter('frequency',1000.0,float,'frequency in MHz'),
        ...
        Parameter('sweep',[
                  ...
                  Parameter('continuous_sweep',False, bool, 'continuous sweep'),
                  ])
    ])
    ...
    def __init__(self, name=None, settings=None):
        ...
        self._send_command(self._param_to_internal('power'), self._power_to_internal(self.settings['power']))   #sets power
        self._send_command(self._param_to_internal('reference'), self._reference_to_internal(self.settings['reference']))   #sets reference
    ...
    def update(self, settings):
        ...
        for key, value in settings.items():
            if not (key == 'address' or key == 'sweep'):
                if key == 'frequency':
    ...
    def _power_to_internal(self, power):
        #converts power level to device value
        if power == -4:
            return 0
        elif power == -1:
            return 1
    ...
    def _internal_to_power(self, power):
        #converts device value power level
        if power == '0':
            return -4
        elif power == '1':
            return -1
...
if __name__ == '__main__':
    ...
    print(usb.read_probes('freq_step'))
    usb.close() 