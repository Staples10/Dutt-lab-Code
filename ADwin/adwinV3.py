...
Added to this version:
    -Removed save_data method. Saved as binary file, I think it would be better to save numbers (in numpy array or whatever in experiment class)
    -Removed set_variables method. You have to define variables in ADbasic script so setting values will also be allocated there
        [could add functionality later Dr. Dutt mentioned RegX and Re as python librarys to read ADbasic script files]
    -clear_process now seperate method
'''
...
class ADwin_Gold(Device):
    ...
    def load_process(self, name, delay=3000):
        '''
        ...
        Args:
            name: file location of ADbasic script
                ...
                -calling load_process(ex_script) will add to process dictionary and allow the same name
                 to be used in start, stop, and clear process methods [start_process(ex_script)]
        ...
        test_script = 'D:/PyCharmProjects/pittqlabsys-main/src/Controller\ADbasic\\Test_controller.TB1'
        '''
    ...
    def clear_process(self, name):
        ...
        Args:
            name: same name used in load_process which will get corresponding number in process dictionary
        '''
