...
To update for next version:
 ...
 -Add mult_ax stop to trigger method
 -Make mult_ax require iterations input
 -Change trigger to trigger(self, key, axis=None)
...
class MCL_NanoDrive(Device):
    ...
    _DEFAULT_SETTINGS = Parameter([Parameter('serial',2850,int,'serial of specific Nano Drive. Dutt labs LP100:2849 & HS3:2850'),
                                   ...
                                   Parameter('mult_ax',[
                                             Parameter('waveform',[[0],[0],[0]],list,'lists for multi axis waveform. Ex: [[x_wf],0,[z_wf]]. Input udpate(settings:{mult:{waveform:[0]]}}) to stop.'),'),
    ...
    def update(self, settings):
        ...
        or update(settings = {'axis':'x','read_waveform':MCL_NanoDrive.empty_waveform}) for reading a waveform
        '''
        ...
        for key, value in settings.items():     #goes through inputed settings to see what commands to send ot update parameters
            ...
            if key == 'position':      #updates axis position
                ...
                load_waveform = ArrayType(*settings['load_waveform'])
                load_rate = self._load_rate_check(self.settings['load_rate'])
                value = self._check_error(self.DLL.MCL_LoadWaveFormN(axis,c_uint(self.settings['num_datapoints']),load_rate,byref(load_waveform),self.handle))
                #To read loaded waveform use update(settings={'axis':'','read_waveform':MCL_NanoDrive.empty_waveform})

            elif key == 'mult_ax' and settings['mult_ax']['waveform'] == [0]:     #stops multi-axis waveform if inputed value is [0]
    ...
    def setup(self, settings):
        ...
        for key, value in settings.items():
            ...
            if key == 'read_waveform':
                ...
                load_waveform = ArrayType(*settings['load_waveform'])
                load_rate = self._load_rate_check(self.settings['load_rate'])
                value = self._check_error(self.DLL.MCL_Setup_LoadWaveFormN(axis,c_uint(self.settings['num_datapoints']),load_rate,byref(load_waveform),self.handle))
