...
To update for next version:
 ...
 -Move read_waveform into read_probes
...
class MCL_NanoDrive(Device):
    ...
    _DEFAULT_SETTINGS = Parameter([Parameter('serial',2850,int,'serial of specific Nano Drive. Dutt labs LP100:2849 & HS3:2850'),
                                   ...
                                   Parameter('clock',[
                                       Parameter('Pixel',[
                                           ...
                                           Parameter('binding','unbind',['x','y','z','aux','read','load'],'axis/event to bind to')
                                       ]),
                                       Parameter('Line',[
                                           Parameter('polarity',0,[0,1],'low=0, high=1'),
                                           Parameter('mode', 0, [0, 1], '0:low-to-high, 1:high-to-low'),
    ...
    def __init__(self, name=None, settings=None, serial=None):
        ...
        self.empty_waveform = [0]    #make an inital empty waveform for read data. Can enter settings={'axis':'','read_waveform':MCL_NanoDrive.empty_waveform} and the empty wavefrom will be updated to correct length etc within method
    ...
    def setup(self, settings):
        ...
        for key, value in settings.items():
            ...
            if key == 'read_waveform':
                ...
                value = self._check_error(self.DLL.MCL_Setup_LoadWaveFormN(axis,c_uint(self.settings['num_datapoints']),load_rate,byref(self.settings['load_waveform'])))
                self.set_load_waveform = True    #lets trigger_load and tirgger_acquisition run

            elif key == 'mult_ax':
                self.load_waveform = self._multiaxis_waveform(self.settings['mult_ax']['waveform'])
    ...
    def clock_functions(self, clock, polarity=None, mode=None, bind_to=None, reset=False, output=False):
        ...
        clock = self._clocks_to_internal(clock)
        if reset:
        ...
        if bind_to != None:
            if polarity == None:
                ...
                bind_to = self._bind_axis_to_internal(bind_to)
                value = self._check_error(self.DLL.MCL_IssBindClockToAxis(clock, c_int(polarity+2), bind_to, self.handle))
    #could add settings = {clock:{'polarity':polariyt}} and super().update(settings) if wanted to track settings
        if output:
            clock_name = f'MCL_{clock}Clock'    #could add more lines to ensure first letter capital and other letters lowercase
            value = self._check_error(getattr(self.DLL, clock_name)(self.handle))  #make sure this works. Other option for formating in Kai's original code
        return None
    ...
    def _check_error(self, value):  #returns inputed value if not an error value. If error value, triggers raise of error encounted
        check_error = self.error_dic.get(value, lambda:value)
        return check_error()
