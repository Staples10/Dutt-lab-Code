...
New to this version:
 -Trigger method takes key instead of settings and added multi axis waveform stop to it
 -New parameter for time_step of multi axis waveform
 -Iterations and time_step now a required input for multi axis waveform setup
 -Axis argument added to update and setup methods in order to make use more intuitive 
  and prevent changing parameter of one axis when another one is intended
 -Improved comments
'''
...
class MCL_NanoDrive(Device):
    ...
    _DEFAULT_SETTINGS = Parameter([Parameter('serial',2850,int,'serial of specific Nano Drive. Dutt labs LP100:2849 & HS3:2850 (20 bit systems)'),
                                   ...
                                   Parameter('mult_ax',[
                                             Parameter('waveform',[[0],[0],[0]],list,'lists for multi axis waveform. Ex: [[x_wf],0,[z_wf]]. Input trigger(mult_ax_stop=True) to stop'),
    ...
    def update(self, settings, axis=None):
        ...
        Args:
            ...
            axis: specific axis to move (can also specify in settings dictionary). If not specified will adjust last interacted with axis
        ex:
            update(settings={'axis':'x','axis_position':5}) or update(settings={'axis_position':5},axis='x') for setting position
            update(settings = {'num_datapoints':100,'load_waveform':waveform) for loading a wave form with 100 datapoints
            update(settings = {'axis':'x','read_waveform':MCL_NanoDrive.empty_waveform}) for reading a waveform
        '''
        ...
        for key, value in settings.items():     #goes through inputed settings to see what commands to send ot update parameters
            if key == 'position':      #updates axis position
                ...
                self.settings['read_waveform'] = list(empty_wf)
    ...
    def setup(self, settings, axis=None):
        ...
        for key, value in settings.items():
            if key == 'read_waveform':
                ...
                self.set_load_waveform = True    #lets trigger_load and trigger_acquisition run
                ...
                if not self.settings['num_datapoints']==len(wf[0])==len(wf[1])==len(wf[2]):
    ...
    def trigger(self, key, axis=None, mult_ax_stop=False):
        '''
        Updates internal settings of NanoDrive and triggers set up commands
        Args:
            key: the key of a parameter in the settings dictionary to specify what setup to trigger ['read_waveform','load_waveform','mult_ax']
            axis: specific axis to move (can also specify in settings dictionary). If not specified will adjust last interacted with axis
            mult_ax_stop=True to stop multi axis waveform
        '''
        if mult_ax_stop:
    ...
    def clock_functions(self, clock, polarity=None, mode=None, bind_to=None, reset=False, pulse=False):
        ...
        Args:
            ...
            reset=True to reset ALL clocks to defaults
            pulse=True to generate a 250ns pulse on specified clock
        '''
        if reset:
    ...
    def read_probes(self, key, axis=None):
        ...
        elif key == 'read_waveform':    #returns waveform read on axis
