...
New to this version:
 -mode/polarity cleared up in defualt settings
 -Make so binding polarity works like normal
'''
    ...
    def clock_functions(self, clock, mode=None, polarity=None, binding=None, reset=False, pulse=False):
        ...
        Args:
            ...
            binding: binds to axis read or event (must specify polarity)
                !!! Flipped polarity for binding: 0 = High-to-Low and 1 = Low-to-High !!!
                ^ Command manual says it should be the same but testing w/ scope indicates difference
        ...
        if reset:
            ...
            reset_settings = {'clock':{'Pixel':{'mode':0,'polarity':0,'binding':'read'}, 'Line':{'mode': 0,'polarity': 0,'binding':'load'}, 'Frame':{'mode':0,'polarity': 0,'binding':'none'}, 'Aux':{'mode':0,'polarity':0,'binding':'none'}}}
        ...
        if binding != None:
            if polarity == None:
                ...
                value = self._check_error(self.DLL.MCL_IssBindClockToAxis(clock, bind_polarity, bind_to_axis, self.handle))
                self.settings['clock'][clock_name]['binding'] = binding
        if pulse:
